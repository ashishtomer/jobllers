    ...
    case listingElements: List[Element] ⇒
      listingElements.foreach{ element ⇒
        ...
        if(dateOfPosting.contains("today") || dateOfPosting.contains("1 day ") || dateOfPosting.contains("hour")) {
          ...
          val jobTitle = element.selectFirst("a.content ul li.desig").text()
          val company = element.selectFirst("a.content span.org").text()
          val experience = element.selectFirst("a.content span.exp").text()
          val location = element.selectFirst("a.content span.loc").text()
